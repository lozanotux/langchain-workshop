...
def process_directory(
    ...
    if os.path.basename(path) == "zh":
        print(
            colored(
                f"The 'zh' directory (Chinese translations) skipped: {path}",
                "yellow"
            )
        )
        return
    ...
    print(
        colored(
            f"Processing directory: {path}"
            f" from the repo: {repo_info['repo']}",
            "blue"
        )
    )
    ...
    if response.status_code == 200:
        ...
        for file in files:
            if file["type"] == "file" and (
                ...
                print(
                    colored(
                        f"Downloading document: {file['name']}",
                        "green"))
        ...
        print(
            colored(
                "Success in extracting documents from the directory.",
                "green"))
    else:
        print(
            colored(

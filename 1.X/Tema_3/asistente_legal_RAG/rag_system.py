...
def get_path_base_name(file_path):
    """Extrae el nombre base de un archivo a partir de su ruta"""
    if not file_path:
        return "No especificada"
    return file_path.split("\\")[-1] if '\\' in file_path else os.path.basename(file_path)
...
def initialize_rag_system():
    # Vector Store
    vectorestore = Chroma(
        embedding_function=MistralAIEmbeddings(model=EMBEDDING_MODEL),
        persist_directory=CHROMA_DB_PATH
    )

    # Modelos
    llm_queries = ChatMistralAI(model=QUERY_MODEL, temperature=0)
    llm_generation = ChatMistralAI(model=GENERATION_MODEL, temperature=0)
        ...
        for i, doc in enumerate(docs, 1):
            ...
            if doc.metadata:
                if 'source' in doc.metadata:
                    source = get_path_base_name(doc.metadata['source'])
    ...
    try:
        ...
        for i, doc in enumerate(docs[:SEARCH_K], 1):
            doc_info = {
                ...
                "fuente": get_path_base_name(doc.metadata.get('source')),

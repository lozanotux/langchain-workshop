...
CHROMADB_PATH = "./Tema_5/chromadb"

# COnfiguracion basica del LLM
llm = ChatMistralAI(model="mistral-large-latest", temperature=0)

# Configuracion de la base de datos vectorial (chromadb)
vectorstore = Chroma(
...
def guardar_memoria(texto):
    """Guarda informacion relevanta del usuario en la base de datos vectorial"""
    try:
        collection.add(
            documents=[texto],
            ids=[str(uuid.uuid4())]
...
def buscar_memoria(consulta, k=3):
    """Busca informacion relevantes en la memoria de chromadb"""
    try:
...
def chatbot_node(state):
    ...
    # 1. Buscar memoria relevantes
    memorias = buscar_memoria(ultimo_mensaje)

    # 2. Crear prompt con memorias
    system_content = """
    Eres un asistente que responde con informacion justa y precisa
    (con la menor cantidad de palabras posible) y que recuerda
    ...
    mensajes_con_sistema = [SystemMessage(content=system_content)] + messages
    response = llm.invoke(mensajes_con_sistema)
...
if __name__ == "__main__":
    ...
    print("\n    (escribe 'memorias' para ver memoria historica)")
